...
describe('generateWhatsAppLink', () => {
  it('generates a wa.me URL with cleaned number and encoded message for valid inputs', () => {
    const link = generateWhatsAppLink('966501234567', 'Modern Apartment', 'prop-123');
    ...
    expect(url.searchParams.get('text')).toBe(
      'Hi, I\'m interested in your property listing: "Modern Apartment" (Ref: prop-123).\nListed on Maskany.',
    );
  });

  it('strips +, spaces, and dashes from the phone number', () => {
    const link = generateWhatsAppLink('+966 50-123 4567', 'Villa', 'id-1');
  ...
  it('URL-encodes the pre-filled message', () => {
    const link = generateWhatsAppLink('966501234567', 'Villa', 'id-1');
  ...
  it('returns null for an empty phone number', () => {
    expect(generateWhatsAppLink('', 'Title', 'id')).toBeNull();
  });
  ...
  it('returns null for a phone number that is too short after cleaning', () => {
    expect(generateWhatsAppLink('12345', 'Title', 'id')).toBeNull();
  });

  it('returns null for a phone number containing non-digit characters', () => {
    expect(generateWhatsAppLink('966abc1234567', 'Title', 'id')).toBeNull();
  });

  it('returns null when the cleaned number starts with zero', () => {
    expect(generateWhatsAppLink('0501234567', 'Title', 'id')).toBeNull();
  });

  it('encodes special characters in the property title', () => {
    const link = generateWhatsAppLink('966501234567', 'Luxury "Seaside" Villa & Spa #1', 'ref/42');
    ...
    const url = new URL(link as string);
    expect(url.searchParams.get('text')).toBe(
      'Hi, I\'m interested in your property listing: "Luxury "Seaside" Villa & Spa #1" (Ref: ref/42).\nListed on Maskany.',
    );
  });
});
