...
function createMockDb(impl: { executeTakeFirst: () => Promise<unknown> }) {
  return {
    selectFrom: vi.fn(() => ({
      select: vi.fn(() => ({
        limit: vi.fn(() => ({
          executeTakeFirst: impl.executeTakeFirst,
        })),
      })),
    })),
...
describe('checkDbHealth', () => {
  it('returns ok when query succeeds', async () => {
    const db = createMockDb({ executeTakeFirst: async () => ({ id: 1 }) });
  ...
  it('returns ok on an empty database (no rows is not a failure)', async () => {
    const db = createMockDb({ executeTakeFirst: async () => null });
    const result = await checkDbHealth(db);
